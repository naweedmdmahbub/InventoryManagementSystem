                        ...
                        <div class="card-header">
                            ...
                            @can('units.create')
                                <div style="text-align: right">
                                    <a class="btn btn-default" href="{{route('units.create')}}">
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                @foreach($units  as $unit)
                                    <tr>
                                        ...
                                        <td>
                                            @can('units.view')
                                                <a href="{{ route('units.show',$unit->id) }}" class="btn btn-warning">View</a>
                                            @endcan
                                            @can('units.edit')
                                                <a href="{{ route('units.edit',$unit->id) }}" class="btn btn-info">Edit</a>
                                            @endcan
                                            @can('units.delete')
                                                <button class="btn btn-danger delete-confirm" data-id="{{ $unit->id }}">Delete</button>
                                            @endcan
                                        </td>
                                    </tr>
                                @endforeach
